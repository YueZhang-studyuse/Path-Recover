...
// for high-level
HNode::HNode(const Config& _C, const Instance& I, DistTable& D, HNode* _parent, const uint _g, const uint _h)
    : C(_C), parent(_parent), neighbor(), g(_g), h(_h), f(g + h), priorities(C.size()), order(C.size(), 0), search_tree(std::queue<LNode*>())
{
    ++HNODE_CNT;
    ...
    else 
    {
        ...
        for (size_t i = 0; i < N; ++i) 
        {
            if (D.get(i, C[i]) != 0) 
            {
                ...
                // if (I.getGuidanceDistance(i,C[i]->index,curr_time) == 0)
                //     priorities[i]++;
            } 
            else 
            {
    ...
    std::sort(order.begin(), order.end(), [&](uint i, uint j) { return priorities[i] > priorities[j]; });
}
...
bool Planner::get_new_config(HNode* H, LNode* L)
{
    ...
    for (uint k = 0; k < L->depth; ++k) 
    {
        ...
        auto l_pre = H->C[i]->id;
        if (occupied_next[l_pre] != nullptr && occupied_now[l] != nullptr && occupied_next[l_pre]->id == occupied_now[l]->id)
            return false;
...
bool Planner::funcPIBT(LACAMAgent* ai)
{
    ...
    std::sort(C_next[i].begin(), C_next[i].begin() + K + 1,
              [&](Vertex* const v, Vertex* const u) 
              {
                ...
                else 
                {
                    ...
                    {
                        v_h = instance.getSecondGuidanceDistance(ai->id,v->index);
                        u_h = instance.getSecondGuidanceDistance(ai->id,u->index);
                    }
    ...
    for (auto k = 0; k < K + 1; ++k) 
    {
        ...
        //std::cout<<"considering "<<i<<" vertex "<<u->index<<" at "<<ai->curr_timestep+1<<endl;
        //std::cout<<"guidance "<<instance.guidance_path[i][ai->curr_timestep+1]<<endl;

        // avoid vertex conflicts
        if (occupied_next[u->id] != nullptr) 
        {
            //std::cout<<"vertex conflict "<<endl;
            continue;
        }
                               ...
                               Vertex* v_pusher_origin, Vertex* v_puller_origin)
{
    ...
    return (D.get(puller, v_pusher) < D.get(puller, v_puller)) &&
          (D.get(pusher, v_pusher) == 0 || D.get(pusher, v_puller) < D.get(pusher, v_pusher));
}
...
bool Planner::is_swap_possible(Vertex* v_pusher_origin, Vertex* v_puller_origin)
{
    ...
    {   // avoid loop
        ...
        {
            auto a = occupied_now[u->id];
            if (u == v_pusher || (u->neighbor.size() == 1 && a != nullptr && ins->goals[a->id] == u)) 
            {
